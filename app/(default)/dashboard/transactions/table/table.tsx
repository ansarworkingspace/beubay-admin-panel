...
import { useTransactions } from "../controller";
...
const FIELD_MAPPING: Record<string, string> = {
    order_id: "search", // Mapping order_id search to the general 'search' param
    status: "status",
    gateway: "gateway"
};
...
export function TransactionsTable() {
    ...
    const memoizedSearchParams = React.useMemo(() => {
        ...
        searchParams.forEach((value, key) => {
            // Keep status in query params if it was navigating from sidebar
            if (key !== 'page' && key !== 'limit') apiParams[key] = value;
        });
        return apiParams;
    }, [searchParams]);

    const { data: responseData, isLoading, error, isFetching, refetch } = useTransactions(page, limit, memoizedSearchParams);
        ...
        <Card>
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <div className="text-sm text-gray-500">
                            Showing {tableData.length || 0} of {paginationInfo?.total || 0} transactions
                        </div>
                    </div>
                </div>
            </CardHeader>
            <CardContent className="space-y-4">
                <DataTableToolbar
                    ...
                    initialSearchValue={columnFilterValues["order_id"] || ""}
                            ...
                            {table.getRowModel().rows?.length ? (
                                ...
                                <TableRow>
                                    <TableCell colSpan={columns.length} className="h-24 text-center">
                                        <p className="text-muted-foreground">{hasActiveFilters ? "No matches." : "No transactions found."}</p>
                                    </TableCell>
                                </TableRow>
                            )}
                        </TableBody>
