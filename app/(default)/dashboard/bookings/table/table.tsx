...
import { useBookings } from "../controller";
...
const FIELD_MAPPING: Record<string, string> = {
    booking_id: "search", // Mapping booking_id search to the general 'search' param as per API requirements
...
export function BookingsTable() {
    ...
    const [columnFilterValues, setColumnFilterValues] = React.useState<Record<string, string>>(() => {
        ...
        searchParams.forEach((value, key) => {
            if (key !== 'page' && key !== 'limit' && key !== 'type') { // 'type' is used for tabs (past/upcoming), keep it separate
                // Reverse map for search
                if (key === 'search') {
                    filters['booking_id'] = value;
                } else {
    ...
    const { data: responseData, isLoading, error, isFetching, refetch } = useBookings(page, limit, memoizedSearchParams);
    ...
    const handleSearchChange = React.useCallback((value: string, columnId?: string) => {
        ...
        debounceTimerRef.current = setTimeout(() => {
            ...
            if (filterValue) params.set(apiKey, filterValue);
        ...
        <Card>
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <div className="text-sm text-gray-500">
                            Showing {tableData.length || 0} of {paginationInfo?.total || 0} bookings
                        </div>
                    </div>
                </div>
            </CardHeader>
            <CardContent className="space-y-4">
                <DataTableToolbar
                    ...
                    initialSearchValue={columnFilterValues["booking_id"] || ""}
                    showGlobalSearch={false}
                // No create button for bookings usually, but if needed adds addTitle and url
                />
                            ...
                            {table.getRowModel().rows?.length ? (
                                ...
                                <TableRow>
                                    <TableCell colSpan={columns.length} className="h-24 text-center">
                                        <p className="text-muted-foreground">{hasActiveFilters ? "No matches." : "No bookings found."}</p>
                                    </TableCell>
                                </TableRow>
                            )}
                        </TableBody>
