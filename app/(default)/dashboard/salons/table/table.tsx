...
import { useSalons } from "../controller";
...
const FIELD_MAPPING: Record<string, string> = {
    salon_name: "salon_name_like",
    owner_name: "owner_name_like",
    is_active: "is_active",
};
...
export function SalonTable() {
    ...
    const { data: responseData, isLoading, error, isFetching, refetch } = useSalons(page, limit, memoizedSearchParams);
    ...
    const table = useReactTable<Salon>({
        data: tableData,
        columns,
    ...
    return (
        <Card>
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <div className="text-sm text-gray-500">
                            Showing {tableData.length || 0} of {paginationInfo?.total || 0} salons
                        </div>
                    </div>
                </div>
            </CardHeader>
            <CardContent className="space-y-4">
                <DataTableToolbar
                    table={table}
                    searchKey="salon_name"
                    searchPlaceholder="Search salon..."
                    onToggleColumnFilters={() => setEnableColumnFilters(!enableColumnFilters)}
                    enableColumnFilters={enableColumnFilters}
                    onSearchChange={handleSearchChange}
                    initialSearchValue={columnFilterValues["salon_name"] || ""}
                    addTitle="Create Salon"
                    url={`/dashboard/salons/create?page=${currentPage}&limit=${currentLimit}`}
                            ...
                            {table.getRowModel().rows?.length ? (
                                ...
                                <TableRow>
                                    <TableCell colSpan={columns.length} className="h-24 text-center">
                                        <p className="text-muted-foreground">{hasActiveFilters ? "No matches." : "No salons found."}</p>
                                    </TableCell>
                                </TableRow>
                            )}
                        </TableBody>
