...
import { useCustomers } from "../controller";
...
export function CustomerTable() {
    ...
    const { data: responseData, isLoading, error, isFetching, refetch } = useCustomers(page, limit, memoizedSearchParams);
        ...
        <Card>
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <div className="text-sm text-gray-500">
                            Showing {tableData.length || 0} of {paginationInfo?.total || 0} users
                        </div>
                    </div>
                </div>
            </CardHeader>
            <CardContent className="space-y-4">
                <DataTableToolbar
                    ...
                    url={`/dashboard/users/create?page=${currentPage}&limit=${currentLimit}`}
                    showGlobalSearch={false}
                />
                {/* Secondary Search (e.g. Phone) if not handled by toolbar: Toolbar usually only has one main search. 
                     If phone search is important, we usually add it as a filter or just assume 'name' search searches multiple fields in backend if implemented.
                     But here I mapped 'name' -> 'name_like'. 
                     I can also add a distinct one, but DataTableToolbar is generic.
                     For now, filtering column 'phone' is possible when filters are enabled.
                 */}
                            ...
                            {table.getRowModel().rows?.length ? (
                                ...
                                <TableRow>
                                    <TableCell colSpan={columns.length} className="h-24 text-center">
                                        <p className="text-muted-foreground">{hasActiveFilters ? "No matches." : "No users found."}</p>
                                    </TableCell>
                                </TableRow>
                            )}
                        </TableBody>
